...
annot_col <- data.frame(Direction = factor(col_anno$direction))
rownames(annot_col) <- colnames(tab)
...
annot_row <- data.frame(Direction = factor(row_anno$direction))
rownames(annot_row) <- rownames(tab)
...
pheatmap::pheatmap(tab, clustering_distance_rows = "correlation",
         ...
         col = hcl.colors(100, "Blues",rev=F),
         show_rownames=TRUE,
         show_colnames=TRUE)
