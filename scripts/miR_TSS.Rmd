...
```{R}
targets <- read.csv("/data/TCGA-PRAD/mirna_res/mirnet_mir_gene.csv", header=T)

key <- genes$hgnc_symbol

targets <- subset(targets, targets$symbol %in% key)
